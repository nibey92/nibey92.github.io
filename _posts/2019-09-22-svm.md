...
그렇다면 제약식이 0이 될때는 $$\vec{w}$$ 가 어디에 존재할까요? 바로 boundary 위에 존재한다는 것을 알 수 있습니다. 
...
`SVM`의 목적은 무엇이였는지 생각해 봅시다. `SVM`의 목적은 `margin`을 최대화 하는 것이였죠. 그렇다면 이 margin이란 것을 수식으로 표현해서 maximize 하면 될 것 같군요. Margin의 길이를 계산하기 위해 아래 그림에서 처럼 boundary 위에 있는 $$\vec{w}$$를 사용하겠습니다.
