...
`SVM`의 목적은 무엇이였는지 생각해 봅시다. `SVM`의 목적은 `margin`을 최대화 하는 것이였죠. 그렇다면 이 margin이란 것을 수식으로 표현해서 maximize 하면 될 것 같군요. Margin의 길이를 계산하기 위해 아래 그림에서 처럼 boundary 위에 있는 $$\vec{x}$$를 사용하겠습니다.

![graph]({{ site.url }}/img/svm-11.JPG)
...
![graph]({{ site.url }}/img/svm-10.JPG)

$$
\begin{align*}
margin &= (\vec{x_+}-\vec{x_-}) \cdot \frac{\vec{w}}{\begin{Vmatrix}\vec{w}\end{Vmatrix}_2}  \\ 
 &= \frac{\vec{x_+}\cdot\vec{w}-\vec{x_-}\cdot\vec{w} }{\begin{Vmatrix}\vec{w}\end{Vmatrix}_2}\\ 
 &= \frac{(1-b)-(-1-b)}{\begin{Vmatrix}\vec{w}\end{Vmatrix}_2}\\ 
 &= \frac{2}{{\begin{Vmatrix}\vec{w}\end{Vmatrix}_2}}
\end{align*}
$$
...
![graph]({{ site.url }}/img/svm-12.JPG)

이렇게 변환하는 것은 단지 수학적 계산의 편의성을 위한 것입니다. norm w가 뜻하는 것이 루트 로 되어있으니 이를 제곱하면 계산이 쉬워질거라는 추측이 가능하죠.
...
![graph]({{ site.url }}/img/svm-13.JPG)


## KKT 조건


![graph]({{ site.url }}/img/svm-14.JPG)


![graph]({{ site.url }}/img/svm-15.JPG)
