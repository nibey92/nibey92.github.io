...
그럼 수식의 값이 0보다 크면 $$\vec{x}$$를 $$\vec{x_+} $$로 분류해야 할까요? 아직은 판단하기 이릅니다. `SVM`에서는 `margin` 영역이 있습니다. $$\vec{x_+} $$와 $$\vec{x_-} $$ 어느 데이터도 들어 있지 않은 구간이죠. 이 구간을 정해줍시다.
...
앞에서 boundary에 대한 식을 보았습니다. 이제 실제 데이터가 있는 공간 `Plust plane`과 `Minus plane`에 속해있는 데이터 벡터들 $$\vec{x_+}$$와 $$\vec{x_-}$$를 가지고 제약식을 정의해 보도록 하겠습니다. 위에서 정한 boundary를 기준으로 생각하면  plus data vector $$\vec{x_+}$$ 들은 plus boundary 위에 있어야 할 것이고 minus data vector $$\vec{x_-}$$들이 살고 있는 공간은 minus boundary밑에 위치해야 겠죠. 따라서 이것을 식으로 그대로 쓰면 다음과 같이 쓸 수 있겠습니다. 

* $$\vec{w} \cdot \vec{x_+} + b >= +1$$
* $$\vec{w} \cdot \vec{x_-} + b >= +1$$
*
